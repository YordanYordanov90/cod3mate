...
export function createBot(deps: BotDependencies): Cod3mateBot {
  ...
  /**
   * Telegram bot.command() only matches commands with [a-z0-9_]. Hyphenated
   * /qa-history style messages are plain text — route them here.
   */
  async function dispatchQaSlashCommand(
    ctx: Context,
    command: string,
    argsText: string
  ): Promise<boolean> {
    switch (command) {
      ...
      case '/qa_history': {
        ...
        if (recent.length === 0) {
          await sendSafe(
            ctx,
            'No QA reports saved yet. Use qa_assert_* tools during a task to generate one.',
            env.TELEGRAM_CHUNK_SIZE
          );
          return true;
        }
      ...
      case '/qa_report': {
        ...
        await sendSafe(ctx, sanitizeString(formatQaReport(stored)), env.TELEGRAM_CHUNK_SIZE);
        return true;
      }
      ...
      case '/qa_scenarios': {
        ...
        lines.push('Run with: /qa-run <name>');
        lines.push(
          'Save new ones by telling the agent "save this flow as <name>" (it will use the qa_save_scenario tool).'
        );
        await sendSafe(ctx, sanitizeString(lines.join('\n')), env.TELEGRAM_CHUNK_SIZE);
        return true;
      }
      ...
      case '/qa_test': {
        ...
        await processAgentTask(ctx, effectiveTask, {
          maxIterations: env.QA_MAX_ITERATIONS,
          collectQaReport: true,
        }, 'qa-test');
        return true;
      }
      ...
      case '/qa_run': {
        ...
        const name = (argsText.split(/\s+/)[0] || '').trim();
        if (!name) {
          await sendSafe(
            ctx,
            'Usage: /qa-run <scenario-name>\n\nList available with /qa-scenarios.\nSave new ones via the qa_save_scenario tool (ask the agent to "save the flow I just did as checkout-flow").',
        ...
        try {
          ...
          if (testCredentials) execOpts.testCredentials = testCredentials;
          if (appCredentials && Object.keys(appCredentials).length > 0) {
            execOpts.appCredentials = appCredentials;
          }
          const { result: execRes, report: qaReport, screenshotPaths } = await withQaReportCollector(
            reportTitle,
            () => executeScenarioSteps(scenario, execOpts)
  ...
  bot.on('message:text', async (ctx) => {
    ...
    if (userText.startsWith('/')) {
      const { command, argsText } = parseSlashCommand(userText);
      if (await dispatchQaSlashCommand(ctx, command, argsText)) {
        return;
      }
      if (command && !knownCommands.has(command)) {
        await sendSafe(
          ctx,
          `Unknown command: ${command}\n\nTry /help — QA reports use /qa-history.`,
          env.TELEGRAM_CHUNK_SIZE
        );
      }
      return;
